...
- 只有登录应用才会在此目录生成对应应用的用户文件夹，你可以把文件拷贝进去来上传文件到应用，从应用中下载的文件最好从此目录拷贝到你的家目录或其他目录打开，因为执行卸载操作的时候容器将全部被删除，此目录里的文件也会被销毁
- 多次运行容器或者应用也不会影响文件存放路径，此目录是所有应用共用的空间，不必担心文件路径冲突等问题
